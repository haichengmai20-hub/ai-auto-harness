...
- **最后更新**：2026-06-16（**审查 be54e47 + CC 同步两批**：①审查上一轮(Q4/Q5/F9/F2/P3,`be54e47`)发现 F9 三个分类用 `sed` 往 Python entry 源码塞 Megatron CLI flag/盲替端口数字 = 毁文件(把 `args`/`argparse`/`sys.argv` 替成注释)→ #41(`fb17a70`)修正为不动文件:te_spec/incompatible_ckpt 分类+诊断转人工、distributed/port 走环境变量、te_missing once-guard;5 分类行为回归 entry 全 byte-identical。②#42(`6aad610`)把已验证安全集 Q4(HF id→本地路径)/Q5(超时分级)/F2(no_proxy localhost)/F9(安全集+改源码红线)同步进 CC `run-and-repair/SKILL.md`(LLM 指令范式,毁灭性 sed 绝不进 CC);P3 经 shared `scripts/reconcile-state.sh` 已生效无需 CC 单独改。③#43(快赢批 #2)清四个局部坑:H3(install-env 第 6.5 步 apt 系统依赖,9 组映射同 Hermes 6d)、F10(install 第 1 步 `bash -n ~/.bashrc` 失败→BASH_ENV=/dev/null)、F7(run-and-repair 收集 ModuleNotFound 批量装+连续 2 轮 dep_missing 深扫)、F8(子进程空 stderr→dmesg 抓 OOM)。剩余:F1 服务型架构簇(F11/F12/F13/F4/F14)+ F3/F5/F6 依赖簇未做。06-12:**#40 续跑假退出修正落地** — khala 48.6GB 实战 4 次续跑空转 300+ 调用 → 新增 `scripts/check-bg-downloads.sh` 单一真相源(僵尸+30min 停滞双检)、daily.sh 入口 WAIT_GATE + 0-token 免配额复查链(10min,FD200 防泄漏,复查触发的启动仍耗配额)、fetch-weights/auto-daily SKILL 快退路径、hermes preflight BG 门同步;对 spec §7 草案修正 4 处(僵尸误判/停滞饿死/次日干等/输出标记耦合);fixture 11 场景全过,待下个大权重项目实战;khala 本体已下完 49G 等次日 cron 接续推进 install。同日:磁盘 cron 门槛 150GB(双框架);**Hermes 版落地(`hermes/`)** — 按 docs/migration-to-hermes.md 实现:主编排 skill + 7 阶段 playbook(delegate_task 派发,S-1 原则 context 指路)、guard.env.sh bash 函数拦截 R1/R4/R6/R7/R11(替代 PostToolUse hook,单测 10/10,R1/R11 直接拒绝比 hook 的"仅警告"更强)、preflight/postflight/monitor/notify 四个 0-token cron 脚本(preflight 双轴 BUSY 防双驱,实测拦下 CC 正跑的 khala)、install.sh 已实装 ~/.hermes 但 **cron job 未注册**(CC crontab 还在跑,--register-cron 有双驱硬拒);4 处与方案的明确偏离及切换/回滚步骤见 hermes/README.md。06-11:#39 SCAIL 批次审查更正:4 真 bug+R4.6/R11 编号+P10-P12 下沉 run-and-repair SKILL;SCAIL 试跑 3 轮 $105 失败→P1-P12 改善落地,daily.sh 已具备 30min 续跑+15min 异常重试;06-10:P0 #36 no_proxy 断网 + P0 #37 sentinel 对账(外部 review 采纳 9/20);**fix 积压清零** — #15-#24 全部闭环:6 条后续工作线已实证落地补记,#18 verify_level 分级 / #22 repair 口径 / #23 poll 按阶段重置 / #24 字面量防呆当日实现;未闭环 0 条,仅余 #30/#31 部分落地;目标 2026-06-11 cron 9:00→10:00 链路无人值守跑通）；**#44 F1 服务型架构簇落地**:helper `scripts/service-lifecycle.sh`(start/wait-ready/stop 可测) + 四阶段 SKILL 改造(intake F13/run F1+F4/verify F11/cleanup F12) + `scripts/reconcile-sentinels.sh` 孤儿回收安全网(F14)；fix 计数 43→44
- **版本**：v1.1（Phase 5 进行中）
