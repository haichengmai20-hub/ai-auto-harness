...
**一句话**：定规则"设计文档怎么管"——正文只写结论不写历史，改了必须加 ChangeLog 引 fix 证据。

**打比方**：像法律条文只写"现在是什么规定"，历史修法过程写在附录里。正文出现"以前是..."就违规。

**核心规则**：
- 正文 + ChangeLog 二分：正文是当下结论，ChangeLog 是变更历史
- 不许在同一主题写第二份 spec（避免"哪份是真"困惑）
- SKILL.md 有双重身份：既是 LLM 的指令，也是人的文档
- 命名规则：spec 用 `spec-<topic>.md`，plan 用 `phase-<N>-<topic>.md`
- 生命周期：draft → active → deprecated（不删文件）
